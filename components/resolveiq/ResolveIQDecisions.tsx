...
export function ResolveIQDecisions() {
  return (
    <section id="decisions" className="py-16 border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {resolveiqData.engineeringDecisions.map((dec, idx) => (
            <GlassPanel
              key={idx}
              variant="default"
              className="p-6 sm:p-8 hover:border-cyan-400/30 transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start gap-2 mb-3 text-cyan-300 font-mono text-xs font-semibold">
                  <HelpCircle className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
                  <span>{dec.question}</span>
                </div>

                <div className="mb-4 inline-block px-3 py-1 rounded bg-cyan-500/10 border border-cyan-400/20 text-xs font-mono font-bold text-white">
